...
CSV、ジャケット画像、音声ファイルは同じフォルダに置いてください。
...
また、csvに記載する音声ファイル、画像ファイルは両方ともファイル名だけを記載してください。
...
# スクリプトをAfterEffectsに取り込んで、次回以降はメニューから実行する場合
1. After Effects を起動します。
2. `ファイル > スクリプト > スクリプトのインストール...` を選びます。
3. `AfterEffects_Script/makeXFD/src/makeXFD.jsx` を選択します。
4. After Effectsを再起動すると、次回以降 `ファイル > スクリプト > `からmakeXFD.jsxが実行可能となっています。
5. 以降の手順は # スクリプトを直接実行する場合 の4.以降と同様です。
...
- 画像ファイルについては、正方形でない場合はエラーを表示して処理を中断します。
